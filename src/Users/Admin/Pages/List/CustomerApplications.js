...
const CustomerApplication = () => {
  ...
  const [activeItem, setActiveItem] = useState('All Pending Customer Applications');

  useEffect(() => {
    fetchData(`http://localhost:3550/admin/customerapplication/readallpending?page=${page}&size=${pageSize}`, setCustomerApplications, setTotalPages);
  }, [page]);
  ...
  const handleView = (customer) => {
    navigate('/admin/customerapplicationdetailview', { state: { customer, activeItem: 'All Pending Customer Applications' } });
  };
  ...
  const handleConfirm = () => {
    if (currentAction && selectedId) {
      ...
      const url = currentAction === 'approve'
        ? `http://localhost:3550/admin/customerapplication/approve/${selectedId}`
    ...
    <div className="dashboard-container">
      ...
      <div className="main-content">
        <div className="table-container">
          <h3>Pending Customer Applications</h3>
          <Table
