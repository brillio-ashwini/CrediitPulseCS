...
describe('CustomerApplication Component', () => {
  ...
  const renderComponent = () => {
    render(
      <MemoryRouter initialEntries={['/admin/customerapplications']}>
        <Routes>
          <Route path="/admin/customerapplications" element={<CustomerApplication />} />
        </Routes>
      </MemoryRouter>
    );
  };

  test('renders CustomerApplication component correctly', () => {
    renderComponent();
  ...
  test('pagination works correctly', () => {
    ...
    expect(require('../../Utils/FetchData/FetchData').fetchData).toHaveBeenCalledWith(
      'http://localhost:3550/admin/customerapplication/readallpending?page=1&size=9',
      expect.any(Function),
      expect.any(Function)
    );
  });
  ...
  test('approve modal opens and confirms action', async () => {
    ...
    await waitFor(() => expect(axios.put).toHaveBeenCalledWith('http://localhost:3550/admin/customerapplication/approve/1'));
